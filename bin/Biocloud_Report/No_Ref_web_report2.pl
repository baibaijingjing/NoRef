...
&USAGE unless ($idir and $data_cfg);
...
if(!exists $pro_inf{pro_sam_receive_date}){
    $pro_inf{pro_sam_receive_date}="xxxxxx";
}
if(!exists $pro_inf{pro_sam_receive_date}){
    $pro_inf{pro_sam_receive_date}="xxxxxx";
}
if(!exists $pro_inf{pro_sam_pass_date}){
    $pro_inf{pro_sam_pass_date}="xxxxxx";
}
if(!exists $pro_inf{pro_launch_date}){
    $pro_inf{pro_launch_date}="xxxxxx";
}
if(!exists $pro_inf{pro_finish_date}){
    $pro_inf{pro_finish_date}="xxxxxx";
}
if(!exists $pro_inf{Customer_info}){
    $pro_inf{Customer_info}="xxxxxx";
}
if(!exists $pro_inf{Contract_NO}){
    $pro_inf{Contract_NO}="xxxxxx";
}

if(!exists $pro_inf{user_unit}){
    $pro_inf{user_unit}="xxxxxx";
#print "aaaaaa $pro_inf{user_unit}";
}
#$pro_inf{project_name}            #福建农林大学荔枝转录组测序分析
#$pro_inf{project_id}              #BMK140701-E16
#$pro_inf{user_name}               #杨晓宏
#$pro_inf{user_unit}               #北京百迈客生物科技有限公司
#$pro_inf{pro_sam_receive_date}    #2014-08-13
#$pro_inf{pro_sam_pass_date}       #2014-08-14
#$pro_inf{pro_launch_date}         #2014-08-15
#$pro_inf{pro_finish_date}         #2014-09-29

mkdir "$idir/biocloud/" unless (-d "$idir/biocloud/");
my $cloud_dir = "$idir/biocloud";

print "\n[".&GetTime($BEGIN_TIME)."] $Script start ...\n";
#######################################################################################

## link data and get sample number
my $sample_number = &data_link_($data_cfg);
...
###############################################样品、分组、物种名
my $T_num=0;
my @samples = glob "$idir/geneExpression/*.geneExpression.xls";
...
my $bio_rep = 0;
my @dir;
foreach my $dir (glob "$idir/DEG_Analysis/*_vs_*") {
	$dir=basename $dir;
	push @dir,$dir;
...
my @name = glob "$idir/Unigene/Unigene_CDS/*.Unigene.cds.distribution.png";
my $name = $name[0] if ($name[0]);
$name =~ s/.*\/Unigene_CDS\/(.*).Unigene.cds.distribution.png/$1/;

########################################################################示例文件
mkdir "$cloud_dir/template" unless (-d "$cloud_dir/template");
my @cp = ("Trinity_workflow.png","base_erro_ratio.txt","RNA-Seq_experimental_workflow.png","RNA-Seq_analysis_workflow.png","cleandata_FASTQ_format.png","FPKM_formula.png","enrichment_factor_formula.png");
foreach (@cp) {
...
###############################################样品测序数据评估统计表
open ALLSAMP,"$idir/cleandata/AllSample_GC_Q.stat";
open ALLSAMPNEW,">$idir/cleandata/AllSample_GC_Q_New.stat";
print ALLSAMPNEW "Samples\tTotal reads\tTotal bases\tGC%\tQ30%\n";
my $clean_data = 0;
my $clean_Q30 = 100;
my @samp_all = @samples;
while (<ALLSAMP>) {
	...
	print ALLSAMPNEW "$samp\t$allsampnew[1]\t$allsampnew[2]\t$allsampnew[3]%\t$allsampnew[7]%\n" if ($samp);
...
my $unigene_1kb;
# -------------------------------------------------
if (-d "$idir/Assembly/All_Combination") { #合并组装
# -------------------------------------------------
	open ASSEMBLYC1,"$idir/Assembly/All_Combination/contigs/All_Combination.contigs.stat.info.xls";
	while (<ASSEMBLYC1>) {
		next if (/^\D+/);
		chomp;
	...
	&assem_per(1000);
	&assem_per(2000);
	open ASSEMBLYC2,"$idir/Assembly/All_Combination/contigs/All_Combination.contigs.stat.xls";
	while (<ASSEMBLYC2>) {
		chomp;
	...
	open ASSEMBLYT,"$idir/Assembly/All_Combination/Transcripts/All_Combination.Transcripts.stat.xls";
	while (<ASSEMBLYT>) {
		chomp;
		&assem_t_u('Trans',$_);
	}
	close ASSEMBLYT;
	open ASSEMBLYU,"$idir/Assembly/All_Combination/Unigenes/All_Combination.Unigenes.stat.xls";
	while (<ASSEMBLYU>) {
		chomp;
		&assem_t_u('Uni',$_);
	}
	...
	open ASSEMOUT,">$idir/Assembly/All_Combination.stat.xls";
...
# ------------------------------------------------------------
	...
	my ($Uni_file1) = glob "$idir/Unigene/*.stat.xls";
	...
	my @Uni_group_file = glob "$idir/Assembly/*/Unigenes/*.Unigenes.stat.xls";
...
###############################################################相关性系数表
my $de_limit=0;
my $de=(glob "$idir/DEG_Analysis/*_vs_*")[0];
if ($de && -d $de) {
#	($de1,$de2)=$de=~/Diff_Analysis\/([^\/]+)_vs_([^\/]+)/;
	$de_limit=1;
}
if (-d "$idir/DEG_Analysis/All_DEG") {
	$de_limit=2;
}
...
if ($de_limit!=0) {
	$ftable = "$idir/geneExpression/free_com.cor";
...
open SWXCF,">$idir/geneExpression/free_com.stat";
...
foreach my $sample_map (@samples) {
	open MAPSTAT,"$idir/geneExpression/$sample_map.Mapped.stat.xls";
	$mapstat .= $sample_map;
	while (<MAPSTAT>) {
		next if (/^\s*$/);
...
close MAPSTAT;
open MAPSTATOUT,">$idir/geneExpression/Total.Mapped.stat";
print MAPSTATOUT $mapstat;
close MAPSTATOUT;

###############################SSR标记数量
open SSR,"$idir/Unigene/Unigene_SSR/$name.Unigene.1000.fa.stat.xls";
my $ssr_num;
while (<SSR>) {
	next unless (/^Total/);
	chomp;
...
close SSR;
###############################Unigene注释数量
my $unigene_anno_num = `wc -l $idir/Unigene/Unigene_Anno/Integrated_Function.annotation.xls`;
...
$report_cfg .=&info("report_version","2");
$report_cfg .=&info("report_name",$pro_inf{Project_name});
#print "aaaaaaaaa  $pro_inf{user_unit}";
$report_cfg .=&info("report_customer",'用户：'.$pro_inf{Customer_info}.' | 单位：'.$pro_inf{user_unit});
$report_cfg .=&info("report_code",$pro_inf{Contract_NO});
...
my $analysis_abstract;

if ($sample_number > 1) {
    $report_cfg .=&info("report_abstract","分析结果概述：&lt;br/&gt;(1) 完成${samples_num}个样品，共${clean_data}Gb的转录组测序数据分析，各样品Q30碱基百分比为$clean_Q30\%及以上。&lt;br/&gt;(2) De novo组装后共获得$assembly{'Uni'}{'T_n'}条Unigene。其中长度在1kb以上的Unigene有${unigene_1kb}条。&lt;br/&gt;(3) 对Unigene进行功能注释，包括与NR、Swiss-Prot、KEGG、COG、KOG、GO和Pfam数据库的比对，共获得${unigene_anno_num}条Unigene的注释结果。&lt;br/&gt;(4) 完成基于Unigene库的基因结构分析，其中SSR分析共获得${ssr_num}个SSR标记。同时还进行了CDS预测分析。&lt;br/&gt;(5) 完成基因在各样品中的表达量分析。&lt;br/&gt;(6) 基于基因在不同样品中的表达量，完成差异表达基因分析。&lt;br/&gt;(7) 完成对差异表达基因进行模式聚类、功能注释以及富集性分析。");
    $analysis_abstract =<<"_EOF_";
(1) 完成${samples_num}个样品，共${clean_data}Gb的转录组测序数据分析，各样品Q30碱基百分比为$clean_Q30\%及以上。
(2) De novo组装后共获得$assembly{'Uni'}{'T_n'}条Unigene。其中长度在1kb以上的Unigene有${unigene_1kb}条。
(3) 对Unigene进行功能注释，包括与NR、Swiss-Prot、KEGG、COG、KOG、GO和Pfam数据库的比对，共获得${unigene_anno_num}条Unigene的注释结果。
(4) 完成基于Unigene库的基因结构分析，其中SSR分析共获得${ssr_num}个SSR标记。同时还进行了CDS预测分析。
(5) 完成基因在各样品中的表达量分析。
(6) 基于基因在不同样品中的表达量，完成差异表达基因分析。
(7) 完成对差异表达基因进行模式聚类、功能注释以及富集性分析。
_EOF_
} else {
    $report_cfg .=&info("report_abstract","分析结果概述：&lt;br/&gt;(1) 完成${samples_num}个样品，共${clean_data}Gb的转录组测序数据分析，样品Q30碱基百分比为$clean_Q30\%。&lt;br/&gt;(2) De novo组装后共获得$assembly{'Uni'}{'T_n'}条Unigene。其中长度在1kb以上的Unigene有${unigene_1kb}条。&lt;br/&gt;(3) 对Unigene进行功能注释，包括与NR、Swiss-Prot、KEGG、COG、KOG、GO和Pfam数据库的比对，共获得${unigene_anno_num}条Unigene的注释结果。&lt;br/&gt;(4) 完成基于Unigene库的基因结构分析，其中SSR分析共获得${ssr_num}个SSR标记。同时还进行了CDS预测分析。&lt;br/&gt;(5) 完成基因在样品中的表达量分析。&lt;br/&gt;(6) 完成基因功能注释分析。");
    $analysis_abstract =<<"_EOF_";
(1) 完成${samples_num}个样品，共${clean_data}Gb的转录组测序数据分析，样品Q30碱基百分比为$clean_Q30\%。
(2) De novo组装后共获得$assembly{'Uni'}{'T_n'}条Unigene。其中长度在1kb以上的Unigene有${unigene_1kb}条。
(3) 对Unigene进行功能注释，包括与NR、Swiss-Prot、KEGG、COG、KOG、GO和Pfam数据库的比对，共获得${unigene_anno_num}条Unigene的注释结果。
(4) 完成基于Unigene库的基因结构分析，其中SSR分析共获得${ssr_num}个SSR标记。同时还进行了CDS预测分析。
(5) 完成基因在样品中的表达量分析。
...
## make analysis abstract file
#$analysis_abstract = decode('UTF-8', "$analysis_abstract\n");
open (FH, ">$idir/../abstract.txt") or die "$!: $idir/../abstract.txt\n";
print FH $analysis_abstract;
close FH;
...
$tab_num++;
my $raw_qua_pic = (glob "$idir/cleandata/PNG/*$samples[0]*.quality.png")[0];
$raw_qua_pic = basename($raw_qua_pic);
$out .=&file("图$pic_num 碱基质量值分布图","","cleandata/PNG/$raw_qua_pic","碱基质量值越高表明碱基识别越可靠，碱基测错的可能性越小。比如，对于碱基质量值为Q20的碱基识别，100个碱基中有1个会识别出错；对于碱基质量值为Q30的碱基识别，1,000个碱基中有1个会识别出错；Q40表示10,000个碱基中才有1个会识别出错。&lt;br/&gt;以测序循环为单位，对单个样品所有Reads平行测序的碱基质量值做分布图，可以查看单个样品各个测序循环及整体的测序质量。","注：横坐标为测序碱基在Reads上的位置，纵坐标为碱基质量值。颜色深浅表示碱基比重，颜色越深，说明该位置测定的碱基中为对应质量值的碱基所占的比重越大，反之亦然。图${pic_num}中，上方的蓝色条越多且颜色越深说明样品整体的测序质量越好。","100k","png","png");
$pic_num++;
$out .='</menu-2>'."\n";
...
$out .=&examplefile("表$tab_num 样品测序数据评估统计表","暂无","cleandata/AllSample_GC_Q.stat",'','注：SampleID：百迈客对样品的统一编号；ReadSum：Clean Data中pair-end Reads总数；BaseSum：Clean Data总碱基数；GC(%)：Clean Data GC含量，即Clean Data中G和C两种碱基占总碱基的百分比；Q30(%)：Clean Data质量值大于或等于30的碱基所占的百分比。',"100k","xls","stat");
$tab_num++;
$out .='</menu-2>'."\n";
...
my $unigen_distribution_png = (glob "$idir/Unigene/*.distribution.png");
$unigen_distribution_png = basename($unigen_distribution_png);

if (-d "$idir/Assembly/All_Combination") {
...
if (-d "$idir/Assembly/All_Combination") {
	...
	foreach  (glob "$idir/Assembly/*/Unigenes/*.Unigenes.fa") {
		my $name = basename($_);
		my $dir = $_;
		$dir =~ s/$idir//;
		$name =~ s/\.Unigenes\.fa//;
...
$out .=&file("图$pic_num Mapped Reads在mRNA上的位置分布图","","geneExpression/Total.randcheck.png",'',"注：横坐标为mRNA位置，纵坐标为对应位置区间内Reads在Mapped Reads中所占百分比。由于参考的mRNA长度不同，作图时将每个mRNA按照长度划分成100个区间，进而统计每一区间内的Mapped Reads数目及所占的比例，图中反映的是所有mRNA各个区间内的Mapped Reads比例的汇总。","100k","png","png");
$out .='</menu-2>'."\n";
$pic_num++;
...
$out .=&file("图$pic_num 插入片段长度模拟分布图","","geneExpression/$samples[0].insertSize.r.png","","注：横坐标为双端Reads在Unigene库中比对起止点之间的距离，范围为0到800bp；纵坐标为比对起止点之间不同距离的双端Reads或插入片段数量。","100k","","");
$out .='</menu-2>'."\n";
$pic_num++;
...
$out .=&file("图$pic_num 转录组测序数据饱和度模拟图","暂无","geneExpression/Total.gene_tag.png",'',"注：通过将Reads等量地分成100份，逐渐增加数据查看检测到的基因数量来绘制饱和度曲线。横坐标为reads数目（以10^6为单位），纵坐标为检测到的基因数量（以10^3为单位）。表达量FPKM不小于0.1的基因为表达的基因。","100k","png","png");
$out .='</menu-2>'."\n";
$pic_num++;
$out .='</menu-1>'."\n";
...
$out .='<ori_data>'."\n";
$out .=&examplefile("ORF预测编码区序列文件","","Unigene/Unigene_CDS/$name.Unigene.cds.fa","","Unigene编码区碱基序列——fasta格式","100k","txt","list");
$out .=&examplefile("ORF预测蛋白序列文件","","Unigene/Unigene_CDS/$name.Unigene.pep.fa","","Unigene编码的氨基酸序列——fasta格式","100k","txt","list");
...
foreach (glob "$idir/SNP_Analysis/*_vs_*") {
	$_ = basename($_);
...
if ($bio_rep == 1) {
	...
	$out .=&info("desc","对于设立生物学重复的项目，评估生物学重复的相关性对于分析转录组测序数据非常重要。首先，生物学重复的相关性可以检验生物学实验操作的可重复性；其次，生物学重复的相关性可以评估差异表达基因的可靠性。最后，生物学重复的相关性可以辅助异常样品的筛查。&lt;br/&gt;将皮尔逊相关系数r（Pearson&apos;s Correlation Coefficient）作为生物学重复相关性的评估指标&references_and_link{(Schulze S K, Kanwar R, G?lzenleuchter M, et al. SERE: Single-parameter quality control and sample comparison for RNA-Seq. BMC genomics Italic, 2012, 13(1): 524.),(http://www.biomedcentral.com/1471-2164/13/524/)}。r^2越接近1，说明两个重复样品相关性越强。&lt;br/&gt;为了使分析更加准确可信，原则上，同一条件的生物学重复样品数目不得少于3个。这是为了能够筛查异常样品，并且在剔除一个异常样品后，保证每个条件至少还有2个生物学重复样品，进而直接进行后续差异表达分析，减小整个实验失败的风险，提高效率。同时，百迈客也保证对同一条件的所有生物学重复样品进行同人同批样品提取、建库，同Run同Lane测序。对异常样品进行详细分析，并根据分析结果与沟通共识决定重新进行实验，还是剔除异常样品进行后续分析。&lt;br/&gt;该项目同一条件任意一对生物学重复样品的r^2统计如表${tab_num}：");
	...
	if (-d "$idir/DEG_Analysis/corr") {
		my $corr = (glob "$idir/DEG_Analysis/corr/corr_result*cor.png")[0];
...
if (-f "$idir/DEG_Analysis/All_DEG_veen.png") {
	my $deg_num = @dir;
	$out .='<menu-2>'."\n";
...
$out .='<diagram>'."\n";
foreach (@dir){
	my @kegg_map = (glob "$idir/DEG_Analysis/$_/pathway/kegg_map/*.png");
...
$out .=&info("desc","分析差异表达基因在某一通路上是否过出现（over-presentation）即为差异表达基因的Pathway富集分析。利用富集因子（Enrichment Factor）分析Pathway的富集程度，并利用Fisher精确检验方法计算富集显著性。其中富集因子的计算公式如下：&lt;br/&gt;&lt;img src=&quot;/report/report/showjobimgByPath?filepath=$idir/biocloud/template/enrichment_factor_formula.png&quot; /&gt;&lt;br/&gt;&lt;br/&gt;差异表达基因的KEGG通路富集分析结果见图${pic_num}：");
$out .=&file("图$pic_num ${deg}差异表达基因KEGG通路富集散点图","暂无","DEG_Analysis/$dir[0]/Graph/$dir[0].KEGG.Phase.png",'',"注：图中每一个图形表示一个KEGG通路，通路名称见右侧图例。横坐标为富集因子（Enrichment Factor），表示注释到某通路的差异表达基因数目在所有注释到该通路的基因总数目中所占的比例。富集因子越小，表示差异表达基因在该通路中的富集水平越显著。纵坐标为Q值的对数值，其中Q值为多重假设检验较正之后的P值。因此，纵坐标越大，表示差异表达基因在该通路中的富集显著性越可靠。","100k","png","png");
$out .='</menu-2>'."\n";
$pic_num++;
...
foreach  (@dir) {
	...
	if (-f "$idir/DEG_Analysis/$_/Cog_Anno/$_.Cog_class.txt") {
...
$out .='<menu-2>'."\n";
$out .=&info("name","上机测序");
$out .=&info("desc","库检合格后，用HiSeq2500进行高通量测序，测序读长为PE100。");
$out .='</menu-2>'."\n";
$out .='</menu-1>'."\n";
...
#######################################################################################
open (OUT,">$idir/configtest.xml") or die $!;
print OUT <<"_END_";
...
#    system "rm -r $cloud_dir/display_cleandata/*";
#
...
#        system "ln -s $data{$sample}{fq1} $cloud_dir/display_cleandata/${sample}_1.fq ";
#        system "ln -s $data{$sample}{fq2} $cloud_dir/display_cleandata/${sample}_2.fq ";
#    }
...
sub file{ #&file('$name','$software','$path','$desc','anno','$size','$type','$actiontype');
	...
	my $type=shift;
	my $actiontype=shift;
	print "$idir/$path file not exists\n" unless (-f "$idir/$path");
	my $filename = basename($path);
	`convert -resize 540x180 $idir/$path $cloud_dir/images/$filename` if (-f "$idir/$path");
	$T_num++;
...
Description: 
     ...
     Usage:
       Options:
            --id    <DIR>   analysis result directory, forced
            --cfg   <FILE>  data config, cleandata path, forced
            --inf   <FILE>  project basic information configration file.        [\$Bin/pro_inf.cfg]
            -h              help
#-------------------------------------------------------------------------------------------------------------------
_USAGE_
	print $usage;
