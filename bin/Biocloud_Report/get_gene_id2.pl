...
=pod
	...
	my@geneE=glob("$para{Basic}/Remap/geneExpression/*.geneExpression.xls");
	my$dir="$para{ANNO}/Integrated_Function.annotation.xls";
	#print STDOUT $dir;
	my%RPKM;
	open OUT,">$fout" or die $!;
	print OUT "#ID\tFPKM\t";
 	open IN ,$geneE[0] or die $!;
	while (<IN>){
		chomp;
		next if /^#/;
		my @tmp=split(/\t/);
...
=cut
	...
	while(<IN>){
		...
		my@tmp=split(/\t/);
		if (/^#/){
			my$head=join("\t",@tmp[1..$#tmp]);
			print OUT "$head\tKEGG_pathway_annotation\n";
			next ;
		};
		$anno{$tmp[0]}=join("\t",@tmp[1..$#tmp]);
		$anncolNum=@tmp;
	}
	my@annNull;
	for (2..$anncolNum) {
	...
	##output
	for my $gene (keys %RPKM){
		my $out="$gene\t$RPKM{$gene}";
		if(exists $anno{$gene}){
			$out.="\t$anno{$gene}";
		}
		else{
