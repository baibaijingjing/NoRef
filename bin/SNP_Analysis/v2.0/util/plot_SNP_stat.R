...
--no.grid	-r 	NULL 		Do not drawing grid [optional, default: TRUE]
...
if ( is.null(opt$no.grid) )  	{ opt$no.grid = TRUE }
...
if (length(unique(df$group))>20){p<-p + guides(col = guide_legend(nrow=20))}
