...
df <- data.frame(pathway_term=data[,opt$pathway_term.col], rich_factor=data[,opt$rich_factor.col], qvalue=data[,opt$qvalue.col],gene_number=data[,opt$gene_number.col])

# order data
df <- df[order(df[,3]),]
#sbuset data
if (!is.null(opt$toplines) ){
	df<-head(df,n=opt$toplines)
}else{
	if (!is.null(opt$q_threhold)){
		#threhold<-get_threhold(df,opt$q_threhold)
		df<-subset(df,qvalue<opt$q_threhold)
	}else{
		df<-head(df,n=20)
	}
}
...
colnames(df)<-c("pathway_term","rich_factor","qvalue","gene_number")
...
if ( is.null(opt$outdir))	{
	...
	print(p)
	dev.off()
	write.table(as.matrix(df),file=paste(opt$outdir,"/",opt$outfile,".list",sep=""),quote=F,sep='\t',row.names=F,col.names=T)
	#setwd(curr_dir)
}
q(save='no')
