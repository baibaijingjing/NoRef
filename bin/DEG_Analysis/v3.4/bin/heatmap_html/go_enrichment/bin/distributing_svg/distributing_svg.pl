...
$ver=4.73;#2007-06-06 13:02 修正对Simple支持不完整的bug；
		  ...
		  #修正多Type角标显示不正常的bug
$ver=4.74;#2009-01-08 15:21 自动转pdf文件，需要svg2xxx_release的路径
$ver=4.75;#2009-03-25 11:35 加入Filter参数，滤掉低于Filter的数值。
...
$g->b("svg","viewBox",
	  "0 0 ".($vbW*$param{WholeScale})." ".(($vbH+$ch*@note)*$param{WholeScale}),
	  "width",$vbW*$param{WholeScale},
	  "height",($vbH+($ch+$yFontSpace)*@note+$yFontSpace*15)*$param{WholeScale},"xmlns","xmlns:xlink");
...
sub draw{
	...
	while (<F>) {
		...
		if (/^([^:]+):([^:]+):?(.*)$/) {
			...
			$other[0]=~s/\s//g;
			$other[1]=~s/\s//g;
			if ($drawType =~ /$type[3]/) { #Point x2,y2 延伸线，好像没什么用
	...
	for ($dotNum=1;$dotNum<=$maxDotNum;$dotNum++) {
		...
		foreach (@x) {
			...
			foreach $i (sort {$xy{$_}{$a}{order}<=>$xy{$_}{$b}{order}} keys %{$xy{$_}}) {
				...
				if (($drawType =~ /$type[0]|$type[1]|$type[2]/) && $draw{Align}=~/C/i) { #Rect=> L: Left; C: Center; R: Right 矩形对齐
					$x -= $w/2;
