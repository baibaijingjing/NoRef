...
$component_file   ||=  "$Bin/database/cellular_component.txt";
$function_file    ||=  "$Bin/database/molecular_function.txt";
$process_file     ||=  "$Bin/database/biological_process.txt";
$foutdir          ||=  './';
$help             ||=  "";
#-------------------------------------------------------------------------------------
&help () if($help);
...
my $dissvg_pl="$Bin/distributing_svg/distributing_svg.pl";
...
if (defined $partdraw) {
	...
	unless (defined $nodraw){
		...
		#perl /share/nas2/genome/bmksoft/tool/svg2xxx/v1.0/svg2xxx *.svg  -t png
