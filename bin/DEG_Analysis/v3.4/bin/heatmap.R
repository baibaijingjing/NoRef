...
.sourcePath<-dirname(get_script_path())
if(file.exists(paste(.sourcePath,"/pheatmap2.r",sep="")))source(paste(.sourcePath,"/pheatmap2.r",sep=""))else stop(paste(.sourcePath,"/pheatmap2.r does not exist!",sep=""))
...
##import data
#cleandata<-read.table(opt$infile,head=T,sep="\t",comment.char = "")
#rawdat<-cleandata[,!is.element(colnames(cleandata),c("FDR","log2FC","regulated"))]
rawdat<-read.table(as.vector(opt$infile),header=T,sep="\t",comment.char = "",check.names = FALSE)
...
if(is.na(opt$groupfile)){
  ...
  row.names(colGroup)<-names(group)#������Ʒ��ɫ��
