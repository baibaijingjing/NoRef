...
if(file.exists(infile)){
  indat<-read.table(as.vector(infile),sep="\t",comment.char = "#",header = F,check.names = FALSE) 
  sample<-paste("dat",1:(ncol(indat)-2),sep="_")
  colnames(indat)<-c("class","funct",sample) 
  bardat<-as.matrix(indat[,sample])
  line1st<-as.matrix(read.table(infile,sep="\t",header = F,comment.char = "",nrows= 2,colClasses = "character",check.names = FALSE))  
...
axis(side = 4,at=c(0,1,2,3)-0.1,lab=floor(yl1),cex=txt.size,tck=0,col="grey",las=1,xpd=T,font = 2)
axis(side = 4,at=c(0,1,2,3)+0.1,lab=floor(yl2),cex=txt.size,tck=0,col="black",las=1,xpd=T,font = 2)
...
text(x=max(le.x),y=3-0.1,labels = sam.lab[1],cex=txt.size,pos=4,font = 2)
text(x=max(le.x),y=3+0.1,labels = sam.lab[2],cex=txt.size,pos=4,xpd=T,font = 2)
