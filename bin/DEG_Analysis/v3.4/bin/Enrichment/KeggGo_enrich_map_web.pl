...
unless ($opts{func} eq "go")
{
	...
	print OUT "\#Pathway",'ko_ID','DEG_in_Pathway','AllGene_in_Pathway','DEG_in_All_pathways','AllGene_in_All_pathways','Gene_id','KEGG_Orthology';
#---------------------------------- by Simon Young 2015-03-04 --------------------------------------
        my $header =<<"_HEADER_";
...
#        print OUT $header;
	...
	foreach (keys %{$kegg{gene}}) {
		my @tmp=(split/\t/,$_);
		next if $tmp[2]<1;                    ##line 317,reduce pathway which deg number is smaller than1;
	...
	$\ = "\n";
	{
		...
		while (<IN>) {
				...
				next if $m <1;                                         ##reduce pathway which deg number is smaller than1;
				my $p_value = &hyper($m,$M,$n,$N);
				my $m_ratio = "$m out of $n ".100 * $m / $n.'%';
				my $M_ratio = "$M out of $N ".100 * $M / $N.'%';
				$hash{$term}{name} = $term;
...
sub extract{
	...
	if ($var=~/ko/) {
		...
		open (OUT,">$go_out/$pre_key") || die "Can't creat $kegg_out/$var,$!\n";
	}
	while (<IN>) {
		if (/^\#/) {
			print OUT $_;
