...
while(<I>){
	...
	if($.==1){
		for(my $i=1;$i<=$#tmp;$i++){
			$kog_mark=$i  if $tmp[$i]=~/KOG_class$/;
		}
		next;
	}
	die "Err:$fIn/Integrated_Function.annotation.xls does not contain KOG_class title\n" unless $kog_mark;
	next if (/^\#/ or /^\s*$/); 
	next unless exists $DEG{$tmp[0]};
	$tmp[$kog_mark]=~s/[\[\]]//g;
	foreach my $elem (split "",$tmp[$kog_mark]) {
		$Data{$elem}++;
	}
} 
...
#`ssh compute-0-14 /share/nas2/genome/biosoft/R/3.1.1/bin/Rscript $Bin/kog_anno_plot.r $od/Kog_Anno/$key.Kog.classfy.stat $od/Kog_Anno/$key.Kog.classfy.png `;
my $Rscript= ${&selectconf("$Bin/./../../../../../config/")}{Rscript};
&timeLog("$Rscript $Bin/anno_plot.r $od/Kog_Anno/$key.Kog.classfy.stat $od/Kog_Anno/$key.Kog.classfy.png \"KOG Function Classification of Consensus Sequence\"");
system("$Rscript $Bin/anno_plot.r $od/Kog_Anno/$key.Kog.classfy.stat $od/Kog_Anno/$key.Kog.classfy.png \"KOG Function Classification of Consensus Sequence\"");
