...
$component_file   ||=  "$sys_cfg{component_file}"; # 2015-9-9 ~
$function_file    ||=  "$sys_cfg{function_file}"; # 2015-9-9 ~
$process_file     ||=  "$sys_cfg{process_file}"; # 2015-9-9 ~
...
##three_go_hash:{2-level go des}{go id that belong to 2-level go}
##three_go_tree_key {1-level des}=2-level go des
##tree_root:1-level go des
&get_file_info(\@opt_i, \@opt_mark, \%file_provide,\@real);
##file_provide{sample}{go id}{gene id}
...
open Z,">$foutdir/$fkey.2GO.DEG_geneID.xls" ||die;
print Z "\#GO_classify2\tDEG_num\tDEG_geneID\n";
foreach my $k (keys %correlation) {
        my $num=0;
        $num = (keys %{$correlation{$k}{"DEG Unigene"}});
        print Z "$k\t$num\t";
        print Z join(";",(keys %{$correlation{$k}{"DEG Unigene"}}));
        print Z "\n";
}
close Z;
...
my $Rscript= $sys_cfg{Rscript};
my $cmd="$Rscript $Bin/GOClassificationMap2.r --infile $foutdir/$fkey.GO_enrichment.stat.xls --outpath $foutdir --key $fkey ";
&timeLog($cmd);
&runOrDie($cmd);
