...
while(<I>){
	...
	if($.==1){
		for(my $i=1;$i<=$#tmp;$i++){
			$cog_mark=$i if $tmp[$i]=~/COG_class$/;
		}
		next;
	}
	die "Err:$fIn/Integrated_Function.annotation.xls does not contain COG_class title\n" unless $cog_mark;
	next if (/^\#/ or /^\s*$/); 
	next unless exists $DEG{$tmp[0]};
	$tmp[$cog_mark]=~s/[\[\]]//g;
	foreach my $elem (split "",$tmp[$cog_mark]) {
		$Data{$elem}++;
	}
}
...
system("$Rscript $Bin/anno_plot.r $od/Cog_Anno/$key.Cog.classfy.stat $od/Cog_Anno/$key.Cog.classfy.png \"COG Function Classification of Consensus Sequence\"");
