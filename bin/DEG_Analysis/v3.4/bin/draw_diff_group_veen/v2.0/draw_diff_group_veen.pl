...
for my $dir (glob "$id/*_vs_*") {
    ...
    my $deg_num = `awk 'END{print NR-1}' $id/$name/$name.DEG_final.xls`;
    chomp $deg_num;

    if ($deg_num==0) {
        print "DEG set $name is null, and will not be used to plotting veen.\n";
    } else {
...
Program Date:   2014-10-29
      Modify:   deal with a little bug, so that accept non-null DEG sets to plot veen only.
