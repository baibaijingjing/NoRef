...
&USAGE unless ( $refseq and ($list or $idir) and $odir);

my @DEG_file = glob "$idir/*_vs_*/*DEG*final.xls";
if (@DEG_file){
...
if ($idir) {
    system "cat $idir/*_vs_*/*DEG*final.xls |cut -f 1 > $odir/$index.id.list ";
} else {
    system "cut -f 1 $list > $odir/$index.id.list ";
}
...
}else{print "No DEG for PPI Analysis!";}
...
Program Date: 2015-05-21
 ...
 Description: This program is used to build protein-protein interactions for DEG, by interolog transfers with STRING database.
       Usage: 
        ...
        Examples:
            perl $Script --qseq Maize.Known.longest_transcript.fa --idir DEG_Analysis/ --clade 4577 --odir DEG_Analysis/PPI_dir/
            perl $Script --qseq Basic_Analysis/Cluster/Unigene/Bee.Unigene.fa --idir DEG_Analysis/ --clade eukaryota --odir DEG_Analysis/PPI_dir/
