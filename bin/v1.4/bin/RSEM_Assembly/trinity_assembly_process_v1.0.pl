...
if ($step == 1) {
	&stepStart(1,"Prepare Shell files & Log file & Config files for Assembly.");

	&MKDIR ("$od/Config");
	if ($Assembly_Combination{Combine}==1) {

#		if ($Assembly_Combination{Com_Data}>60) {
#			#push @{$Data_log{$Assembly_Combination{Com_Data}}}, "All_Combination";
#			my $jm=($Assembly_Combination{Com_Data}*1.3)."G";
#			my $cmd="perl $Bin/bin/normalize_read_coverage.pl -i $data_config -k normal -datas $Assembly_Combination{Com_Data} -max_cov 50 -JM $jm -d $od/normalized_reads";
#			$cmd.=" -nlz_cpu $Assembly_Para{para_normalize_cpu}" if exists $Assembly_Para{para_normalize_cpu};
#			&runOrDie("$cmd");
#			my ($fq1_cat,$fq2_cat);
#			open (IN,"$od/normalized_reads/normal.normalized.data.config") or die $!;
#			while (<IN>) {
#				chomp;
#				$fq1_cat=(split/\s+/,$_)[1] if /^fq1/;
#				$fq2_cat=(split/\s+/,$_)[1] if /^fq2/;
#			}
#			close IN;
#			my $config = "$od/Config/All_Combination.config";
#			my $conf;
#			&creat_trinity_config ($config,"All_Combination",$fq1_cat,$fq2_cat,\%Assembly_Para,$conf);
#			my $data_s=(-s $fq1_cat)+(-s $fq2_cat);
#			$data_s = int($data_s/(10**9)/3);
#			push @{$Data_log{$data_s}}, "All_Combination";
#		}
#		else {
			push @{$Data_log{$Assembly_Combination{Com_Data}}}, "All_Combination";
			...
			my $config = "$od/Config/All_Combination.config";
			my $conf;
			&creat_trinity_config ($config,"All_Combination",$fq1_list,$fq2_list,\%Assembly_Para,$conf);
...
#		$grid_type =4 if  ($data_s <= 12);
		foreach my $config_pre (@{$Data_log{$data_s}}) {
			...
			my $cmd="mkdir -p $od/Trinity_assembly/$config_pre && perl $Bin/bin/trinity_process.pl -cfg $od/Config/$config_pre.config -od $od/Trinity_assembly/$config_pre  ";
	...
	foreach my $shell (@shells) {
		...
		while ($status eq "Null") {
			...
			if ($status eq "Null") {
				...
				threads->create(sub{system " sh $status";});
			}
		}
	}
	for my $thread (threads->list(threads->list(threads::all))){
		$thread->join();
...
{ #load config file
	...
	while (<IN>) {
		...
		if ( /^Sample/ ) {
			...
			if (! -e $fq1 || ! -e $fq2) {
				die"Non-exist: Reads file dir for $sample_name isn't exists\n$fq1\n$fq2\n" ;
			}
...
sub grid_run_cmd {
	...
	if (exists $Assembly_Para{para_normalize_cpu}){
		open OUT,">$sh.run.sh" || die $!;
		my $headnode=$sys_cfg{headnode};
		print OUT "ssh $headnode 2>/dev/null <<TT\n";
		print OUT "ssh $Assembly_Para{para_normalize_cpu} 2>/dev/null <<EOF\n";
