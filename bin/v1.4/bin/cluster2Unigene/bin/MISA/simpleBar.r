...
if ( !is.null(opt$no.grid) ) {
	...
	p <- p + theme_bw()+ theme(legend.position = "none",panel.grid.major = element_blank(), panel.grid.minor = element_blank());
}
# remove legend
...
# output plot
#-----------------------------------------------------------------
scale <- opt$height/600
#pdf(file=paste(opt$outfile,".pdf",sep=""), height=opt$height*2/1000, width=opt$width*2/1000)
#print(p)
#dev.off()
png(filename=opt$outfile, height=opt$height, width=opt$width, res=100*scale, units="px")
print(p)
dev.off()
