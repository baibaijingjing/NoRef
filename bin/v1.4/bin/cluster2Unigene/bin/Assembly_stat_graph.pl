...
&USAGE unless ($Index and $fa and $region and $od) ;
...
open (OUT0,">$od/$Index.stat.xls");
print OUT0 "$Index Length\tTotal Number\tPercentage\n";
...
open OUT2,">$od/$Index.stat.info.xls" || die $!;
...
foreach (0..31) {
	...
	my $len_span="$min"."~"."$max";
	my $percent;
	if ($site==31 && defined $length_region_dis{31}) {
		$len_span=">3000";
		$percent=($length_region_dis{31}/$count)*100;
		printf OUT2 "%s\t%s\t\t%.2f\n",$len_span,$length_region_dis{31},$percent;
	}
...
close OUT;
chdir $od;
&runOrDie("perl $svg2xxx $Index.distribution.svg");  #svg-->png
...
sub Length_Distribution_svg {#
	...
	$svg.=&svg_txt_middle($width/2,$height-50,16,"#000000","Length (bp)");
	$svg.=&svg_txt($left_pos-10,$low_pos+5,10,"#000000","0");
	$svg.=&svg_txt_middle($width/2,$high_pos/2,18,"#000000","$lable Length Distribution");
	...
	for (1..30) {
		...
		$svg.=&svg_txt_middle($left_pos+($x_step*$_)+5,$xy_coordinate+13,10,"#000000",100*$_,3);
	}
	$svg.=&svg_txt_middle($right_pos,$xy_coordinate+15,10,"#000000",">3000",3);
	...
	for my $site (0..30) {
		...
		$svg.=&svg_rect_event($x,$low_pos-$y,$x_span,$y,"#0000FF","#0000FF",1);
	}
	$svg.=&svg_end();
	return ($svg);
}
...
Usage:
  ...
  -s               <str>    Length Region for Fasta file Stat(300,500,1000,2000)      must be given;
  -od              <str>    OUT file DIR                                              must be given;
  -syscfg          <file>   system config for the third party software                     optional;
