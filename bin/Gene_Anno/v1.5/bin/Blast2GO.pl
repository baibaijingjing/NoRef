...
# GetOptions
# ------------------------------------------------------------------
my ($od,$Index,$id,$worksh,$cpu,$step,$queue);
GetOptions(
				"help|?" =>\&USAGE,
				"od:s"=>\$od,
				"id:s"=>\$id,
...
my $blast2go=$sys_cfg{blast2go};
my $obo=$sys_cfg{go_obo_file};
my @files=sort glob("$id/*.xml");

open (SH,">$worksh/blast2go.sh")||die "$!";
my $j=1;
my $java=$sys_cfg{java};
...
if ($step==1) {
	&qsubOrDie("$worksh/blast2go.sh",$queue,$cpu,"5G");
	$step=2;
}

if ($step==2) {
	system("rm $od/$Index.annot");
