...
my $cmd="$sys_cfg{Rscript} $Bin/GOClassificationMap.r --infile $foutdir/$fkey.GO_enrichment.stat.xls --outpath $foutdir --fileName  $fkey.GO ";
&runOrDie($cmd);
...
=cut
#######################################################################################
my $totalTime=time()-$BEGIN_TIME;
&timeLog("$Script Done.Total elapsed time : ${totalTime}s");
#######################################################################################
