...
my $cog= "$anno_dir/$opts{key}.Cog_class.txt";
my $kog="$anno_dir/$opts{key}.Kog_class.txt";
my $eggNOG="$anno_dir/$opts{key}.eggNOG_class.txt";
...
foreach (keys %{$Anno_Stat{All}}) {
	...
	foreach (sort keys %Anno_Base) {
        ...
        elsif (/eggNOG/ && !defined $Anno_Gene{$id}{eggNOG}) {
            print OUT "\t--\t--";
        }
