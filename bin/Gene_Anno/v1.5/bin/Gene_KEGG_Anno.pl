...
# ------------------------------------------------------------------
my ($id,$Blast_cpu,$Blast_e,$Database,$gene,$odir,$HELP,$queue);
my @anno;
...
&USAGE if (!defined $id || !defined $gene || !defined $odir || !defined $Database || $HELP) ;


###-----------------------------------###
# all the program are in this dir                    
chomp $Bin;
my $blastx = ${&readconf("$Bin/../../../../config/sys.cfg")}{blast}."/blastx";  # lium: 2015-8-21
my $kegg_png = ${&readconf("$Bin/../../../../config/sys.cfg")}{kegg_png_file};
unless (-e $blastx) {
	 die"Error:$blastx in the script $0 is not right!\n";
}
#================================================
&MKDIR($odir);
$id=&ABSOLUTE_DIR($id);
...
foreach my $subfile (@subfiles) {
	...
	#print OUT "$blastall -b 100 -v 100 -p blastx -e $Blast_e -F F -d $Database -i $subfile -a 2 -o $odir/KEGG_Dir/$name.Kegg.blast && \n";
	print OUT "$blastx -task blastx-fast -num_alignments 10 -evalue $Blast_e -db $Database -query $subfile -outfmt 5 -num_threads 2 -out $odir/KEGG_Dir/$name.Kegg.blast.xml && \n";
}
close OUT;

####################run the shell file
&Cut_shell_qsub("$blast_shell_file",$Blast_cpu,"6G",$queue);
system "perl $Bin/bin/blast_format.pl -id $odir/KEGG_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --kegg -queue $queue";
my $anno_file=$Database.".anno";
system "perl $Bin/bin/kegg_tab2path_ko.pl -tab $Tab_dir/$Q_name.Kegg.blast.tab.best -od $Result_dir -key $Q_name.Kegg -anno $anno_file" ;
system "perl $Bin/bin/kegg_map_web.pl -d $gene -i $Result_dir -o $Result_dir -map $kegg_png";
