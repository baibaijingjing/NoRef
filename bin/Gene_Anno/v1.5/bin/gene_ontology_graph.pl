...
my $day=$Times[3];
#######################################################################################
&timeLog("$Script start at:[$Time_Start]\n";
#######################################################################################
# ------------------------------------------------------------------
# GetOptions
# ------------------------------------------------------------------
#---------------------------------options----------------------------------------------
my ($component_file,$function_file,$process_file,$foutdir,$help,$nodraw,@opt_i,@opt_mark,$partdraw,$fkey);
my @opt_mark_id;#simon
...
#$component_file   ||=  "/share/nas1/yangxh/database/GO/releases20120916/GO_Classify/cellular_component.txt"; # 2012-09-16 ~ 2014-12-12
#$function_file    ||=  "/share/nas1/yangxh/database/GO/releases20120916/GO_Classify/molecular_function.txt"; # 2012-09-16 ~ 2014-12-12
#$process_file     ||=  "/share/nas1/yangxh/database/GO/releases20120916/GO_Classify/biological_process.txt"; # 2012-09-16 ~ 2014-12-12
my %sys_cfg=%{&read("$Bin/../../../../config/sys.cfg")};
#component_file   ||=  "/share/nas1/yangxh/database/GO/releases20141211/GO_Classify/cellular_component.txt"; # 2014-12-12 ~ 
#function_file    ||=  "/share/nas1/yangxh/database/GO/releases20141211/GO_Classify/molecular_function.txt"; # 2014-12-12 ~ 
#process_file     ||=  "/share/nas1/yangxh/database/GO/releases20141211/GO_Classify/biological_process.txt"; # 2014-12-12 ~ 

$component_file   ||= $sys_cfg{"component_file"};
$function_file    ||= $sys_cfg{"function_file"};
$process_file     ||= $sys_cfg{"process_file"};
...
foreach (@opt_mark){
    push @opt_mark_id,$_."_gene_num",$_."_gene_ids";#simon
	s/\_/ /g;
	$_ = ucfirst $_;
}
...
my $dissvg_pl=$sys_cfg{distributing_svg};
my $svg2xxx=$sys_cfg{"svg2xxx"};
...
foreach my $k (keys %correlation) {
	foreach my $k1 (keys %{$correlation{$k}}) {
		...
		$out_hash{$k}{$k1} = $num;
        $out_hash_id{$k}{$k1} = join ";",(keys %{$correlation{$k}{$k1}});#simon
	}
}
...
close STAT;
#----------------------------output statistics by simon---------------------------------
open STAT,">$foutdir/$fkey.GO_enrichment.stat.xlsx" || die;
print STAT "#GO_classify\t",join("\t",@opt_mark_id),"\n";
foreach my $k (@tree_root) {
	print STAT ">$k\n";
	foreach my $k1 (@{$clean_key{$k}}) {
		print STAT "$k1";
		foreach my $k2 (@opt_mark) {
			$out_hash{$k1}{$k2} ||= 0;
			print STAT "\t$out_hash{$k1}{$k2}\t$out_hash_id{$k1}{$k2}";
		}
		print STAT "\n";
	}
