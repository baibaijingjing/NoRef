...
my @datas;
my $data_sum;
my $svgxxx = ${&readconf("$Bin/../../../../../config/sys.cfg")}{svg2xxx};
...
open( my $fh, '>', "$fout.tmp" ) or die "cannot open file $fout.tmp, $!\n"; 
select $fh; 
binmode $fh; 
print $graph->burn(); 
close($fh); 
...
while(<I>){
	chomp;
	if(/class="keyText"\>(.+)\s+\[(\d+)\]\s+\<\/text\>/){
		my $data=$2;
		my $ration=sprintf("%.2f",$data/$data_sum*100);
		$_=~s/\[$data\]/\[$data~$ration\%\]/;
		print O "$_\n";
	}
	else{
		print O "$_\n";
