...
my $blastx = "$sys_cfg{blast}/blastx";  # xugl 2015-3
...
foreach my $subfile (@subfiles) {
	...
	#print OUT "$blastall -b 100 -v 100 -p blastx -e $Blast_e -F F -d $Database -i $subfile -m 7 -a 2 -o $odir/Nr_Dir/$name.nr.blast.xml && \n";
	print OUT "$blastx -task blastx-fast -max_target_seqs 100 -outfmt 5 -evalue $Blast_e -db $Database -query $subfile -num_threads 2 -out $odir/Nr_Dir/$name.nr.blast.xml && \n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Nr_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --nr -queue $queue`;
