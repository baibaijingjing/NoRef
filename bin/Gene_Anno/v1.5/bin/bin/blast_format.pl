...
# GetOptions
# ------------------------------------------------------------------
my ($findir,$foutdir,$fshelldir,$fmiddledir,$Nr,$Nt,$Swissprot,$TrEMBL,$Cog,$Kog,$Kegg,$Eval,$queue,$eggNOG);
GetOptions(
				"help|?" =>\&USAGE,
				"id:s"=>\$findir,
				"middir:s"=>\$fmiddledir,
...
if (@blastm0files) {
    ...
    &qsubOrDie("$fshelldir/Covertm0_2Tabbest.sh",$queue,$Cpu,"1G");
    &qsubOrDie("$fshelldir/Covertm0_2Tab.sh",$queue,$Cpu,"1G");
}
...
if (@blastm7files) {
    ...
    &qsubOrDie("$fshelldir/Covertm7_2Tabbest.sh",$queue,$Cpu,"1G");
    &qsubOrDie("$fshelldir/Covertm7_2Tab.sh",$queue,$Cpu,"1G");
}
...
sub Cut_shell_qsub {#Cut shell for qsub 1000 line one file
	...
	if ($line<=1000) {
		#system "sh /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh $shell --queue $queue --maxproc $cpu --resource vf=$vf --independent --reqsub ";
	}
	if ($line>1000) {
		...
		foreach my $div_file (@div) {
			#system "sh /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh $div_file --queue $queue --maxproc $cpu --resource vf=$vf --reqsub ";
		}
	}
}
