...
#my $blastall = "/share/nas2/genome/biosoft/ncbi-blast/2.2.31/bin/blastn";  # lium: 2015-8-21
my $blastn = $sys_cfg{blast}."/blastn";# xugl 2016-3
...
foreach my $subfile (@subfiles) {
	...
	#print OUT "$blastall -b 100 -v 100 -p blastn -e $Blast_e -F F -d $Database -i $subfile -a 2 -o $odir/Nt_Dir/$name.nt.blast && \n";
	print OUT "$blastn  -num_descriptions 100 -num_alignments 100 -evalue $Blast_e -db $Database -query $subfile -num_threads 2 -out $odir/Nt_Dir/$name.nt.blast && \n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Nt_Dir -eval $Blast_e -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --nt -queue $queue`;
