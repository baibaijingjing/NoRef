...
foreach my $subfile (@subfiles) {
	...
	#print OUT "$blastall -b 100 -v 100 -p blastx -e $Blast_e -F F -d $Database -i $subfile -m 7 -a 2 -o $odir/Swissprot_Dir/$name.Swissprot.blast.xml && \n";
	print OUT "$blastx -task blastx-fast -max_target_seqs 100 -outfmt 5 -evalue $Blast_e -db $Database -query $subfile -num_threads 2 -out $odir/Swissprot_Dir/$name.Swissprot.blast.xml && \n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Swissprot_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --swissprot -queue $queue`;
