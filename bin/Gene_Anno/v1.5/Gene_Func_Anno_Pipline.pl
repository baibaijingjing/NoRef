...
# ------------------------------------------------------------------
my ($cfg,$odir);
my ($nr,$nt,$Swissprot,$TrEMBL,$GO,$Cog,$Kog,$Pfam,$Kegg,$all,$eggNOG);
...
$Query||=$CFG{mRNA};
unless (-e $Query) {
    print BOLD RED "ERROR: unigene sequence file is not exist.\n";
    &USAGE;
}
...
if (( !defined $nr)&& ( !defined $nt) && ( !defined $Cog) && ( !defined $Kog) && ( !defined $Pfam) && ( !defined $Swissprot) && ( !defined $TrEMBL) && ( !defined $Kegg) && (! defined $GO) && (!defined $all) && (!defined $eggNOG)) {
...
# ------------------------------------------------------------------
if ($step == 1) {
	&stepStart(1,"cut query FASTA file");
	unless(-e "$mid_dir/cut_fa.Check"){
		...
		&timeLog("cut query FASTA file has been finished.");
	}
    $step = 2;
	&stepTime(1);
}
...
# ------------------------------------------------------------------
if ($step == 2) {
    &stepStart(2,"align with databases.");
    my $SH="";
	my @work_sh;
    if (defined $nr||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print "        nr: $CFG{nr} \n";
        print SH "perl $Bin/bin/Gene_Nr_Anno.pl -id $mid_dir -Database $CFG{nr} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
	if (defined $Kegg||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_KEGG_Anno.pl -gene $Query -id $mid_dir -Database $CFG{Kegg} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
	if (defined $nt||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Nt_Anno.pl -id $mid_dir -Database $CFG{nt} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    if (defined $Cog||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Cog_Anno.pl -id $mid_dir -Database $CFG{Cog} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    if (defined $Kog||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Kog_Anno.pl -id $mid_dir -Database $CFG{Kog} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    if (defined $eggNOG ||defined $all) {
        ...
        open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_eggNOG_Anno.pl  -id $mid_dir -Database $CFG{eggNOG} -od $odir -cpu $Blast_cpu -Blast_e $Blast_e -queue  $CFG{queue}\n";
        close SH;
        push @work_sh,$SH;
    }
    ...
    if (defined $Pfam||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Pfam_Anno.pl --idir $mid_dir --pfam $CFG{Pfam} --odir $odir --cpu $hmmscan_cpu -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    if (defined $Swissprot||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Swissprot_Anno.pl -id $mid_dir -Database $CFG{Swissprot} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    if (defined $TrEMBL||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_TrEMBL_Anno.pl -id $mid_dir -Database $CFG{TrEMBL} -od $odir -Blast_cpu $Blast_cpu -Blast_e $Blast_e -queue $CFG{queue}\n";
		close SH;
		push @work_sh,$SH;
    }
    &Shell_run (\@work_sh);
...
# ------------------------------------------------------------------
if ($step == 3 && ($GO||$all)) {
    &stepStart(3,"start Blast2GO based on nr blast result.");
    &timeLog("perl $Bin/bin/Blast2GO.pl -id $odir/Nr_Dir/ -od $Result_dir -sh_dir $sh_dir -k $Q_name -queue $CFG{queue}");
    system  "perl $Bin/bin/Blast2GO.pl -id $odir/Nr_Dir/ -od $Result_dir -sh_dir $sh_dir -k $Q_name -queue $CFG{queue}";
    $step = 4;
	&stepTime(3);
}
...
# ------------------------------------------------------------------
if ($step == 4) {
	&stepStart(4,"Intergrate annotation results");
    ## result integrate
    if((defined $Cog || defined $Kog || defined $nr || defined $nt || defined $Swissprot || defined $TrEMBL || defined $Kegg || defined $eggNOG) || defined $all){
        &stepStart(4,"Anno Integrate.");
        &timeLog("perl $Bin/bin/anno_integrate.pl -gene $Query -id $Result_dir -od $Result_dir -key $Q_name");
        system("perl $Bin/bin/anno_integrate.pl -gene $Query -id $Result_dir -od $Result_dir -key $Q_name");
    }

    ## GO histogram plot
    if ($GO||$all) {
        &timeLog("Draw GO Graph:");
        &timeLog("perl $Bin/bin/draw_GO_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir ");
    ...
    if ($Cog||$Kog||$all) {
        &timeLog("Draw COG & KOG Graph:");
        &timeLog("perl $Bin/bin/draw_COG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir") if ($Cog||$all);
        system "perl $Bin/bin/draw_COG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir " if ($Cog||$all);
        &timeLog("perl $Bin/bin/draw_KOG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir") if ($Kog||$all);
        system "perl $Bin/bin/draw_KOG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir " if ($Kog||$all);
    }

    ## nr pie plot
    if ($nr||$all) {
		&timeLog("perl $Bin/bin/util/nr_pie_stat.pl -i $Result_dir/$Q_name.nr.anno.txt -o $Result_dir/$Q_name.nr.lib.stat -m 10");
...
sub LOAD_PARA {
	...
	while (<IN>) {
       ...
       unless ($para_key eq 'mRNA'){
		$para_value =~s/^\/.+\/database/$db_node{$random}\/database/;
		while(!-e $para_value &&$para_value=~/database/){
			print "$para_value does not exist,change database\n";
			$random = int(rand(scalar keys %db_node));
			$para_value =~s/^\/.+\/database/$db_node{$random}\/database/;
		}
	   }
        ...
        if ($para_key eq "nr" or $para_key eq "Cog" or $para_key eq "Kog" or $para_key eq "Pfam" or $para_key eq "Swissprot" or $para_key eq "Kegg" or $para_key eq "nt" or $para_key eq "TrEMBL" or $para_key eq "eggNOG") {
            unless (-f $para_value) {
...
sub ABSOLUTE_DIR { #$pavfile=&ABSOLUTE_DIR($pavfile);
	...
	if (-f $in) {
		...
		die"Warning just for file or dir in $in\n";
	}
...
Description: alignment with fuctional database and annotate genes. v1.1.0-based,
	...
	Example:
	  perl $Script --query Maize.Unigene.fa --all --cfg Gene_Func_Anno_Pipline.cfg --od ./
