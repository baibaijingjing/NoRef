...
# ------------------------------------------------------------------
my ($cfg,$od,$min_kmer_cov,$Assembly_dir,$Result_dir,$step,$worksh);
GetOptions(
				"help|?" =>\&USAGE,
				"cfg:s"=>\$cfg,
...
#$min_kmer_cov = $para{'K_cov'};
#$max_number_of_paths_per_node = $para{"path-node"};
...
#my $trinity_HOME = "/share/nas2/genome/biosoft/trinity/trinityrnaseq_r2013-02-25"; # 2014-12-09 ~ 2014-12-25
#my $trinity_HOME = "/share/nas2/genome/biosoft/trinity/r20131110";                  # 2014-12-25 ~ 
...
if ($step==1) {
	...
	if ($lib_type =~ /none/) {
		if (exists $para{jaccard_clip}){
			...
			if($file_size<=30){$cmd.=" --no_normalize_reads ";}
		}
		else{
		...
		if (exists $para{jaccard_clip}){
			$cmd="perl $trinity_HOME/Trinity --seqType $seqType --SS_lib_type $lib_type --max_memory $JM --CPU $p --min_contig_length $min_contig_length --left $fq1 --right $fq2 --output $Assembly_dir --min_kmer_cov $min_kmer_cov --group_pairs_distance $fragment_length --bflyHeapSpaceMax 20G --no_cleanup --bflyGCThreads $p --jaccard_clip --no_distributed_trinity_exec --no_version_check " ;
                        if($file_size<=30){$cmd.=" --no_normalize_reads ";}
		}
		else{
...
if ($step==2) {
	&stepStart(2,"Trinity phase 2: Parallel Assembly of Read Clusters");
        ...
        system "cp $Assembly_dir/recursive_trinity.cmds $Assembly_dir/chrysalis/Phase2TrinityCommands";}else{print "Warnings: Phase2 Trinity cmd not exists!\n";}
