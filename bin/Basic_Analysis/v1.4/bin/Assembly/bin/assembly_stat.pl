...
# ------------------------------------------------------------------
my ($Index,$fa,$region,$od,$upper_len);
GetOptions(
				"help|?" =>\&USAGE,
				"i:s"=>\$Index,
				"s=s"=>\$region,
...
open IN,"$fa" || die $!;
open OUT,">$od/${Index}_normal_len.fa" || die $!;
$/='>';
while (<IN>) {
	chomp;
	s/^\s+//;s/\s+$//;s/\r+$//;
...
#Stat output
################################
open OUT,">$od/$Index.exceed_upper_length.fa" || die $!;
...
system "rm $od/$Index.exceed_upper_length.fa" if (-e "$od/$Index.exceed_upper_length.fa" and -z "$od/$Index.exceed_upper_length.fa"); # delete empty file
...
system "perl $svg2xxx $Index.distribution.svg";  #svg-->png

###############Time
&timeLog("$programe_dir finished.");
