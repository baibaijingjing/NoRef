...
if ($cfg{'Analysis_path'} ne '') {
	...
	my $assembly = glob "$cfg{'Analysis_path'}/Cluster/Unigene/*.stat.xls";
	my @gene_div;
	open ASSEMBLY,$assembly or die $!;
	$assembly=~/Unigene\/(.*).stat.xls/;
	$demo=$1;
	while (<ASSEMBLY>) {
		chomp;
		if (/^N50/) {
			$assemble{$demo}{'N50'} = &format_figure((split /\t/,$_)[-1]);
		}
		if (/^200\-/) {
			$assemble{$demo}{'300'}=(split /\t/,$_)[-1];
			$assemble{$demo}{'300'}=&percent($assemble{$demo}{'300'});
		}
		if (/^Total Length/) {
			$assemble{$demo}{'length'} = &format_figure((split /\t/,$_)[-1]);
		}
		if (/^Total Number/) {
			$assemble{$demo}{'number'} = &format_figure((split /\t/,$_)[-1]);
		}
	}
	close ASSEMBLY;
...
#				$stat{$sample}{'rand'}+=($1-1)**2;  #DepthVariance
	...
	foreach my $sample (sort keys %stat) {
		...
		print OUT "$assemble{$demo}{'N50'}\t$assemble{$demo}{'300'}\t$assemble{$demo}{'length'}\n";
	}
	close OUT;
}
