...
# ------------------------------------------------------------------
my %opts;
GetOptions(\%opts,"Q=i","config=s","outdir=s","byraw=i","h" ,"queue=s") or &USAGE;
# check
...
our $svg2png     =${&readconf("$Bin/../../config/sys.cfg")}{svg2xxx};
our $fastq_qc_stat = "$Bin/bin/fastq_qc_stat";
...
sub create_work_shell{
	...
	foreach my $sample_name (sort keys %$SamData) {
		...
		print OUT "$Rscript $Bin/bin/plot_acgtn.R infile=$od/$sample_name.acgtn outfile=$od/PNG/$sample_name.acgtn.png &&";
		#print OUT "perl $GC_Q_SVG_pl  $od/$sample_name.quality $od/$sample_name.acgtn $od/ &&";
		print OUT "$Rscript $Bin/bin/quality_bar.R infile=$od/$sample_name.quality outfile=$od/PNG/$sample_name.quality.png \n";
...
sub calling_qsub{
	...
	# calling qsub
	&qsubOrDie("$od/work_sh/GC_Q_svg.sh",$queue,10,"10G");
	#`sh /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh $od/work_sh/GC_Q_svg.sh --queue $queue --reqsub --maxproc $qsubNum `;
}
