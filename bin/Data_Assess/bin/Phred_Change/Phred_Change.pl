...
my (%data_cfg);
my $cmd;
mkdir "$od/Qphred_Data" unless -d "$od/Qphred_Data";
...
if ($#all_q==0) {
    ...
    foreach my $file1(@file){
            ...
            my $newfile="$od/Qphred_Data/$filename";
...
foreach my $sample_id(sort keys %{$data_cfg{rawdata}}){
    ...
    if (exists $data_cfg{rawdata}{$sample_id}{fq2}) {
        ...
        print CONF "raw_fq1\t$data_cfg{rawdata}{$sample_id}{fq1}\n";
    if (exists $data_cfg{rawdata}{$sample_id}{fq2}) {
        print CONF "raw_fq2\t$data_cfg{rawdata}{$sample_id}{fq2}\n";
    }
