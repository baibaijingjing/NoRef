...
if ( is.null(opt$outfile) )  { opt$outfile =paste(c(getwd(),"/"),collapse = "")}
...
data <- read.delim(opt$infile,sep="\t",header = T,comment.char="")
# check dim
#sample<-as.vector(data[,2])
sample=data[,grepl("Sample",x = colnames(data))]
pie.data<-as.matrix(data[,-grepl("Sample",x = colnames(data))]) 
pie.data<-data.frame(Adapter=as.numeric(pie.data[,grepl("^Adapter",x = colnames(pie.data))]),Low_quality=as.numeric(pie.data[,grepl("^Inferior",x = colnames(pie.data))]))
...
pieplot<-function(x,legend=names(x),labels=NULL,title="",outfile="pie.plot",height=3000,width=4000,legend.pos="right",col="Set1",is.axis=F,lab.size=5,legend.size=10,no.grid=T,legend.ncol=1,bg=NA){  
  ...
  df$yat=yat
  df$labels=labels
  p <- ggplot(df, aes(x="", y=y, fill=x)) + geom_bar(width = 1, stat="identity")
...
for(i in 1:nrow(pie.data)){
  ...
  outname<-paste(sample[i],".rawDataStat",sep="");
  outfile<-paste(as.vector(opt$outfile),outname,sep="/")
  labels=paste(x,"%",sep="")
